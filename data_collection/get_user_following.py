...
def save_to_json_file(json_response, news_source, label, filename, tweet_id):
    if len(json_response) !=0:
        if not os.path.exists(load_path + news_source + '//' + label + '//' + filename + '//' + 'user_following'):
            os.mkdir(load_path + news_source + '//' + label + '//' + filename + '//' + 'user_following')
        with open(load_path + news_source + '//' + label + '//' + filename + '//' + 'user_following' + '//' + tweet_id, 'w') as outfile:
...
def main():
    ...
    for source in news_sources:
        for label in labels:
            ...
            for filename in list_news_files: #[53:]
                print(filename)
                try:
                    ...
                    pass
            print(f'user followings related to {label} news of {source} collected')
