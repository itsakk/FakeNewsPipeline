...
def create_url(user_id):
    url = 'https://api.twitter.com/2/users/{}?user.fields=created_at,description,entities,id,location,name,pinned_tweet_id,profile_image_url,protected,public_metrics,url,username,verified,withheld'.format(user_id)
    return url
...
def save_to_json_file(json_response, news_source, label, filename, tweet_id):
    if len(json_response) !=0:
        if not os.path.exists(load_path + news_source + '//' + label + '//' + filename +'//' + 'user_profile'):
            os.mkdir(load_path + news_source + '//' + label + '//' + filename + '//' + 'user_profile')
        with open(load_path + news_source + '//' + label + '//' + filename + '//' + 'user_profile' + '//' + tweet_id, 'w') as outfile:
